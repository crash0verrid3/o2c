...
#define RESPONSIVENESS	2	// range is 1 to 10;  Controls acceleration and deceleration power adjustments in Autonomous mode
#define BREAKCOEFFICIENT	0.01	// This is used to calculate how much power is needed to bring the robot to dead stop
									// at the desired target location.  It accounts for factors like robot mass, drivetrain
									// effeciency, drag, wheel type, etc.
...
#define LIFTER_ON_CHANNEL	6
#define LIFTER_OFF_CHANNEL	7
...
static struct RobotStateBuffer	// holds the state of all Robot Subsystems
	...
	int		Autonomous_State;			// 0=uninit; 1=scenario_selected; 2=running; 3=finished.
	...
	double	Navigation_GyroCenter;		// gyro center point (calibrated for no spin) in millivolts
	double	Navigation_GyroNoise;		// gyro signal range considered noise (MEMS produce noise) in millivolts
	double	Navigation_GyroCWsens;		// gyro sensitivity when spinning clockwise in picovolts
	double	Navigation_GyroCCWsens;		// gyro sensitivity when spinning counter-clockwise in picovolts
	double	Navigation_GyroCWfast;		// gyro sensitivity correction for fast spins when spinning clockwise in picovolts
	double	Navigation_GyroCCWfast;		// gyro sensitivity correction for fast spins when spinning counter-clockwise in picovolts
