...
void AUTONOMOUS :: SelectScenario()
	...
	if (SelectedScenario != pMyRobotState->Autonomous_Scenario)		// a new Scenario has been selected
		...
		pMyRobotState->Autonomous_State = 0;
		for (int s = 0; s < SelectedScenario; s++)		// set the starting_index to the start of the selected scenario
			{
		...
		for (step = 0; step < pScenario->NumberOfSteps; step++)					// load the selected scenario steps
			{
...
void AUTONOMOUS :: MoveToPosition()
	{
	float xdiff, ydiff, xpower, ypower, brakepower, scale_speed_to_distance, joystick_angle, scale_to_normalize, direction;
	...
	if (pScenario->Steps[step].Timeout <= AutoTime)
		...
		{
		xdiff = pMyAutoTarget->Robot_Position_X - pMyRobotState->Robot_Position_X;		// field oriented vector lengths
		...
		direction = ((90 + pMyRobotState->Robot_Heading) * DEGREES_TO_RADIANS) - atan2(xdiff,ydiff);
