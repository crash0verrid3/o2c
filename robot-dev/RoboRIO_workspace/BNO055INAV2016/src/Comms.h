...
public:
	COMMS();
	void Init(RobotStateBuffer *pRobotState, INPUT *pInput, RobotStateBuffer *pTargetState, FieldStateBuffer *pFieldState);
	void Get();
	void Send();
...
private:
	...
	Joystick *Joy1;
	Joystick *Joy2;
	RobotStateBuffer ReportState, ReportTarget, *pMyTargetState;
	INPUT			 ReportInput;
	FieldStateBuffer ReportField;
	int commsgetcnt;
};
