...
//				&& (! MyGyro->Write((uint8_t) GYRO_Reg_SMPLRT_DIV, (uint8_t) GYRO_SampleDiv))
	...
	pMyRobotState->Navigation_GyroCenter = GyroCenter_Z * 1000;
	pMyRobotState->Navigation_GyroNoise = GyroNoiseLimit;
	pMyRobotState->Navigation_GyroCWsens = cw_sensitivity * 1000000000;
	pMyRobotState->Navigation_GyroCCWsens = ccw_sensitivity * 1000000000;
	pMyRobotState->Navigation_GyroCWfast = SpinEffectCW * 1000000000000;
	pMyRobotState->Navigation_GyroCCWfast = SpinEffectCCW * 1000000000000;
...
void NAVIGATOR :: Init(RobotStateBuffer *pRobotState, INPUT *pInput)
	{
	SmartDashboard::PutNumber("NavInit", 1);
	...
	InitMyThermostat();
	InitMyAccelerometer();
	SmartDashboard::PutNumber("NavInit", 2);
