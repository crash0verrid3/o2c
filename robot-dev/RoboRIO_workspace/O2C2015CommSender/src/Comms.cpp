...
void COMMS :: Init(RobotStateBuffer *pRobotState, INPUT *pInput)
{
	...
	sizeofstatus = sizeof(TestReportState[0]);
	SmartDashboard::PutNumber(TestStateBuffer[Comms_Init].Title,TestStateBuffer[Comms_Init].Value.intValue++);
	pMyRobotState = pRobotState;
	pMyInput = pInput;
	pMyInput->SprintModeBtn = false;
	memcpy(TestReportState, TestStateBuffer, sizeofstatus * Buffer_Length);
	TestReportState[Comms_Init].Value.intValue++;
	...
	/*	With Send() function, no need to call SmartDashboard here
...
//	return;			// disabled for now
	...
	for (int i=0; i<Buffer_Length; i++)
		{
		switch (TestStateBuffer[i].DisplayType)
			{
			case Boolean:
				if (TestReportState[i].Value.boolValue != TestStateBuffer[i].Value.boolValue)
					SmartDashboard::PutBoolean(TestReportState[i].Title,TestReportState[i].Value.boolValue);
				break;

			case Double:
				if (TestReportState[i].Value.doubleValue != TestStateBuffer[i].Value.doubleValue)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.doubleValue * pow(10,TestReportState[i].DecimalShift));
				break;

			case String:
				if (TestReportState[i].Value.stringValue != TestStateBuffer[i].Value.stringValue)
					SmartDashboard::PutString(TestReportState[i].Title,TestReportState[i].Value.stringValue);
				break;

			case Char:
				if (TestReportState[i].Value.int8Value != TestStateBuffer[i].Value.int8Value)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.int8Value);
				break;

			case Short:
				if (TestReportState[i].Value.shortValue != TestStateBuffer[i].Value.shortValue)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.shortValue);
				break;

			case Int:
				SmartDashboard::PutNumber(TestStateBuffer[Totalruns].Title,TestStateBuffer[Totalruns].Value.intValue);
				if (TestReportState[i].Value.intValue != TestStateBuffer[i].Value.intValue)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.intValue);
				break;

			case Long:
				if (TestReportState[i].Value.longintValue != TestStateBuffer[i].Value.longintValue)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.longintValue);
				break;

			case Float:
				if (TestReportState[i].Value.floatValue != TestStateBuffer[i].Value.floatValue)
					SmartDashboard::PutNumber(TestReportState[i].Title,TestReportState[i].Value.floatValue * pow(10,TestReportState[i].DecimalShift));
				break;
...
//		memcpy(TestReportState[i].Value.stringValue, TestStateBuffer[i].Value.stringValue, sizeofvalue);
		}
	memcpy(TestReportState, TestStateBuffer, sizeofstatus * Buffer_Length);
//	memcpy(TestReportState, TestStateBuffer, Buffer_Length);

//	if (TestReportState[i] != TestStateBuffer[i])
