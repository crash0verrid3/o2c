...
void MECANUMDRIVE :: Init(RobotStateBuffer *pRobotState, INPUT *pInput)
	...
	SmartDashboard::PutNumber("SprintMode", sprint_factor);
	SmartDashboard::PutNumber("OrientMode", OrientMode);
	SmartDashboard::PutNumber("SpinControlPower", rotation);
	SmartDashboard::PutNumber("DriveInit", ++driveinitcnt);
	spin_enable_count = 0;
	StopMotors();
	}
...
void MECANUMDRIVE :: Drive()
	...
	pMyInput->DriveX *= (sprint_factor * XPOWERFACTOR);
	pMyInput->DriveY *= sprint_factor;
	pMyInput->DriveR *= sprint_factor;
	// Now scale X and Y so neither is greater than 100%
	double scalar = 1 / fmax(1, (fmax (fabs (pMyInput->DriveX), fabs (pMyInput->DriveY))));
	pMyInput->DriveX *= scalar;
	pMyInput->DriveY *= scalar;
	...
	SmartDashboard::PutNumber("SpinControlPower", rotation);
	SmartDashboard::PutNumber("DriveTrainHeading", pMyRobotState->Drivetrain_Heading);

	//Call the MecanumDrive_Cartesian RobotDrive Function
	pRobotDrive->MecanumDrive_Cartesian(pMyInput->DriveX, pMyInput->DriveY, rotation, OrientAngle);
	}
...
void MECANUMDRIVE :: SpinControl()
	...
	//		double spin_prediction = (pMyRobotState->Robot_Spin_Rate * LOOPPERIOD * GYRO_CW_SENSITIVITY / 6);
	//		SmartDashboard::PutNumber("SpinPrediction", spin_prediction);
